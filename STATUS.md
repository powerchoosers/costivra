...
- Google OAuth branding is now complete with the approved Costivra mark, `https://costivra.ai` home page, public Privacy and Terms links, and the existing `costivra.ai`/Supabase authorized domains. The external app is published and reports `In production`; Google still requires a formal verification submission before the restricted `gmail.readonly` access is approved and branding is shown without an unverified-app boundary.
- Microsoft Entra reports the mailbox app as activated with one secret, one web redirect, and delegated Graph access. Publisher verification cannot proceed while the tenant publisher domain is `lewispatterson3outlook.onmicrosoft.com`; Microsoft requires a DNS-verified custom tenant domain that matches the primary contact domain of a verified Microsoft Partner Center account, plus that account's MPN ID.
