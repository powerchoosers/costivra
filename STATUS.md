...
- Extended the shared Back-control handoff to every tabbed detail shell: generic App records (including Findings), App Vendor records, and Manage Account/Contact records now explicitly identify their local record navigation. The compact floating Back control stays hidden while those tabs are visible and returns only after they leave the workspace header area, preventing it from competing with local navigation. The Manage record sections now reuse the shared view switcher, rather than a bespoke tab implementation.
...
- Tightened the shared Back-control handoff on record pages. Its logical sticky state is now separate from its temporary visual suppression behind record tabs, tabs hidden under the workspace header no longer block it, and the App control sits in the shell gutter instead of over a Finding card. Focused browser QA confirmed the compact control at `264px` with the App rail open and `84px` closed, clear of record content.
