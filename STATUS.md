...
- Routed the menu toggle, close button, backdrop, Escape key, and navigation links through the same close lifecycle.
- Validation: Node `v24.19.0`; TypeScript, focused ESLint, navigation tests (6 passed), and `git diff --check` passed. Existing App `<img>` optimization warnings remain.

## August 30, 2026 — Tablet navigation drawer styling

- Restored the App mobile navigation drawer’s header, grouped navigation links, active state, icons, and account/sign-out action at the tablet breakpoint. The regression affected 820px-wide layouts because only the phone breakpoint styled the drawer’s children.
- Validation: Node `v24.19.0`; TypeScript, focused ESLint, navigation tests, and `git diff --check` passed. Local browser QA at 820×1180 confirmed the drawer renders as a structured touch menu with visible link spacing and account action.
...
- Made App table routes viewport-bound at portrait-tablet widths; the page chrome stays fixed while the vendor/table wrapper owns touch scrolling and overscroll containment.
- Added an authenticated `auth-820x1180` Playwright project so the complete App/Manage authenticated suite will cover the portrait-iPad layout when credentials are enabled.
- Extended the authenticated shell test to assert the App and Manage bottom docks replace their desktop rails and maintain zero horizontal overflow at 820×1180.
- Extended the touch-first shell breakpoint through 1199px and coarse-pointer tablets so landscape iPads also receive the bottom dock, More drawer, and viewport-bound scroll model. Validation: Node `v24.19.0`; TypeScript no-emit and `git diff --check` passed.
- Full Vitest regression on the current responsive branch: 224 test files passed, 4 skipped; 938 tests passed, 6 skipped. Authenticated browser visual QA remains credential-gated.
- Production build completed successfully with Next.js 16.2.12: TypeScript finished, 62 static pages generated, and all App/Manage route families compiled.
- Added an `auth-1180x820` landscape tablet Playwright profile for the authenticated workspace. The public marketing drawer regression remains intentionally scoped to the 820px portrait profile because the marketing header keeps desktop navigation at 1180px.
- Simplified the Manage portrait-tablet header by hiding the persistent search field and exposing the existing focused search sheet behind a magnifying-glass button; desktop search remains unchanged. Validation: targeted ESLint, TypeScript no-emit, and `git diff --check` passed.
- Refined the portrait-tablet Manage header after authenticated browser QA: search now sits to the right of notifications with a vertical divider, and the focused search sheet is visible and usable at 820×1180. The header reports zero horizontal overflow.
- Repaired the shared Manage touch navigation cascade: dock-item, active-state, drawer-group, link, footer, and motion styles now apply to both phones and tablets instead of stopping at 780px. Tablet navigation is a centered 640px dock with a compact two-column More sheet. Authenticated QA at 908×838 confirmed zero page, dock, or drawer overflow.
- Right-aligned the Manage drawer operator and sign-out control so it hugs the footer's trailing edge while Settings remains anchored on the left.
- Removed the collapsed desktop-rail reservation from the customer App at tablet widths. The iPad shell now uses the full viewport: no 76px left offset, no 12px trailing frame padding, and no desktop topbar corner rounding, while route content retains deliberate internal card spacing.
- Refined the shared Ask Costivra surface motion across App and Manage. Drawer open/close and full-screen expand/minimize now use explicit opening, transitioning, and closing phases; the surface remains mounted until its real animation completes; Manage's close button, Escape handling, scrim, and topbar request share the same lifecycle; and reduced-motion users still receive an immediate state change. Authenticated browser QA at 820×1180 confirmed interpolated drawer/full-screen widths and animated exits in both platforms. Validation: Node `v24.19.0`; focused motion Vitest (9 tests), changed-file ESLint, TypeScript no-emit, production build (62 static pages), and `git diff --check` passed.
- Removed the nested focus ring from the Manage tablet search input while preserving the outer search-field container and its accessible focus treatment. The input now remains visually flat inside the single shared search shell. Validation: authenticated browser QA at 820×1180, TypeScript no-emit, and `git diff --check` passed.
- Unified the Manage touch header and dock geometry across iPad Air, iPad Mini, and phone widths. The title and search divider now survive the 768px breakpoint, the header has a clearer overlay shadow, the iPad dock expands with the viewport, and overview content scrolls beneath the fixed dock while retaining end-of-scroll clearance. Added authenticated regression coverage for the 820px dock geometry and the 768px/390px title-divider contract. Validation: Node `v24.19.0`; authenticated browser QA passed at 820×1180, 768×1024, and 390×844 with zero horizontal overflow and no console errors; TypeScript no-emit, changed-test ESLint, and `git diff --check` passed. The focused authenticated Playwright case reached the suite but is credential-gated because the local `E2E_SUPABASE_SECRET_KEY` is not a real value.
- Corrected the tablet marketing-drawer CTA selector so “Start with 3 bills” receives the intended 24px separation below the Sign in row instead of touching its divider. Validation: Node `v24.19.0`; browser QA at 908×838 and 390×844 confirmed an exact 24px gap, zero horizontal overflow, a clean framework state, and no console warnings/errors; TypeScript no-emit, changed-test ESLint, focused tablet navigation Playwright (1/1), and `git diff --check` passed.

## Shared App and Manage appearance

- Added one persisted Light, Dark, or System preference shared by the customer App and internal Manage workspace. A pre-paint bootstrap applies the saved choice before hydration to prevent a light-theme flash, while the runtime provider follows operating-system changes when System is selected.
- Added searchable `Branding & appearance` sections to both Settings hubs with an accessible theme selector, live effective-theme status, and responsive preview. The preference is intentionally device-local and does not change tenant data or organization branding for other users.
- Extended the shared workspace token system and legacy compatibility bindings across authenticated shells: page canvases, navigation, settings, cards, controls, tables, frozen bulk/identity columns, drawers, mobile docks, Manage inspectors, and neutral status surfaces now remain coherent in dark mode while semantic state colors stay distinct.
- Completed a second dark-mode color pass using the approved dark-background assistant artwork. Settings title/back-link contrast is clearer; Settings search and the Command Center composer retain one visible control surface; attention rows, semantic badges, activation steps, expanded sidebar controls, and compact mobile header controls no longer inherit legacy light fills. Browser QA at 1186×838 and 390×844 confirmed transparent inner input backgrounds, a shell-owned composer focus ring, distinct pending/completed activation states, dark sidebar/header controls, and zero mobile horizontal overflow.
- Added purpose-built transparent SVG variants for the primary Costivra mark and assistant evidence signal so their formerly navy geometry remains legible against the low-light shell. Dark mode selects the light-structure variants while Light continues to use the original approved artwork; the App sidebar Settings dividers now use the dark shell border color. Browser QA at 1538×838 and 390×844 confirmed the correct theme-specific asset URLs, clean console output, a successful Light-to-System interaction, and zero horizontal overflow.
- Completed the vendor-directory dark table pass at the shared primitive layer. Inline attention/healthy badges now use dark semantic surfaces, vendor search owns one dark focusable shell, nested table amounts inherit primary dark text, and the corrected fallback-logo selector uses a dark identity tile. The authenticated logo fallback is now transparent instead of baking in a white rectangle, allowing the same safe initial artwork to sit on Light or Dark surfaces without filtering real vendor brands. Browser QA at 1538×838 and 390×844 confirmed all corrected computed colors, successful vendor filtering, clean logs, five restored rows, and zero page overflow.
- Corrected dark table hover and frozen-column depth at the shared App and Manage table layer. Vendor, Bills, and Contracts rows now keep one restrained dark hover across both fixed and scrolling cells even when late legacy route styles are present; the frozen identity edge now combines a crisp divider with a soft depth gradient, and Manage fixed columns use the same stronger dark separation. Browser QA on `/app/vendors` at 1538×838 with the table scrolled 320px confirmed one computed hover color across every cell, the 18px frozen-edge gradient, and clean console logs.
- Completed a full authenticated dark-mode sweep across App and Manage. Shared dark owners now cover route content, detail workspaces, semantic badges, file/evidence browsers, integrations intake, mail controls, global search results, first-run banners, notifications, create/profile menus, body-portaled App dialogs, Manage creation sheets, and mobile search/navigation overlays. Automated browser scans covered 23 primary routes plus representative vendor, bill, contract, finding, account, contact, intake, and invoice-review details; interaction QA covered App/Manage menus, notifications, global search, dialogs, mobile search, and both navigation drawers. The 390×844 mobile pass reported zero horizontal overflow, and the 1538×838 vendor-table pass confirmed one dark hover color plus a visible frozen-identity divider and depth shadow. Source email and PDF canvases intentionally remain white to preserve document fidelity. Validation: Node `v24.19.0`; TypeScript no-emit, focused workspace-preference Vitest (1 file, 6 tests), and `git diff --check` passed.
- Closed the remaining body-portaled modal gaps found by interaction QA. The vendor-monitoring intake note now uses a shared theme-aware callout instead of an inline light fill; the source-upload picker, dropzone, vendor menu, progress/error/upgrade states, custom date and date-time controls, and Manage account/search context surfaces now own complete dark palettes. Browser surface scans found no opaque light descendants in the monitoring, upload, contract, calendar, or Manage account side-panel families at 1538×838, and the corrected controls retain the existing reduced-motion behavior.
- Validation: Node `v24.19.0`; changed-file ESLint passed; TypeScript no-emit passed; focused workspace-preference Vitest passed (1 file, 6 tests); browser QA passed for dark App Settings and Bills, dark Manage Settings and Accounts, explicit Light mode, cross-shell persistence, and 390×844 dark mobile Settings with zero horizontal overflow. Browser logs reported no errors or warnings. The original `System` preference was restored after QA.
