...
- Extended the signed Resend webhook to keep delivery states current and route `RESEND_OWNER_INBOX` into the owner mailbox while preserving the separate customer document-intake path.
...
- Remaining setup: configure `COSTIVRA_INTERNAL_ADMIN_EMAILS`, `RESEND_WEBHOOK_SECRET`, `RESEND_OWNER_INBOX`, and `RESEND_FROM_EMAIL` in Vercel; register the signed webhook; enable receiving on the dedicated subdomain only after the Resend plan/domain and malware scanner requirements are resolved. No live test email has been sent.
...
- Added one private `inbound.costivra.ai` intake address per organization, automatic provisioning for every new customer workspace, tenant-scoped inbound event and attachment records, Row Level Security, audit events, and required foreign-key indexes in the live Costivra Supabase project.
...
- The production Resend webhook is not registered yet. The handler is ready at `https://costivra.ai/api/webhooks/resend`, but its signing secret, Vercel variables, and receiving domain must be configured before inbound delivery can be enabled.
- Resend account inspection found the current Free plan is limited to one custom domain, already used by `costivra.ai` sending. Receiving was not left enabled on the root domain because its MX record could interfere with the existing business mailbox. Production activation therefore requires Resend Pro (currently $20/month) or another separately approved inbound-email provider so `inbound.costivra.ai` can be registered safely.
