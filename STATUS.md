...
- Removed legacy App and Manage rail scrollbar paint/hide rules so both left-navigation scrollports now use the one shared `data-workspace-scrollbar` contract: transparent at rest, muted public-site yellow-green only while the matching axis is active, then a short fade out. The shared contract reserves the native gutter to prevent the prior gray-to-green width jump. Validation is pending current type and CSS diff checks; fresh local authenticated visual review remains blocked by the browser session redirecting protected routes to `/login`.
