...
- Password recovery confirmation — the intermediate “Continue securely” control now uses an explicit same-site navigation instead of a form submission, because secured/embedded browser contexts can block form POSTs. Supabase verification still requires a second-page `confirm=1` signal that is absent from the email link, preserving the scanner-safe boundary.
