...
- Validation: Node `v24.19.0`; direct `tsc --noEmit --pretty false` and focused ESLint passed. Focused Vitest passed 26 tests across navigation history, record-detail, finding-presentation, and workspace-preference suites. `git diff --check` passed with only line-ending notices. Fresh authenticated browser re-verification is not claimed: the in-app browser rejected the localhost route under its URL policy, so the previously recorded desktop browser checks remain historical evidence only.
