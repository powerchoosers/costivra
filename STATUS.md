...
- Corrected the collapsed App rail brand alignment by removing the stray 12px vertical transform from its 72px header, matching the centered Manage mark. Validation: Node `v24.19.0`; `git diff --check` passed; the restarted local App route returned 200 and browser geometry measured a 0px logo-to-header center delta. The local server is running in webpack mode because a clean Turbopack start could not resolve its internal Google-font module after cache reset.
