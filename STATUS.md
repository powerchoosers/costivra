...
- Before final submission, corrected the live-policy prerequisites locally: the Privacy Policy now has a current Google/Gmail data section and Limited Use statement; the Settings integration shows an in-product privacy notice; OpenRouter requests enforce `data_collection: "deny"` plus per-request zero-data-retention routing; and disconnect now deletes the tenant-scoped OAuth connection, encrypted tokens, and cascading vendor rules instead of retaining token ciphertext in a revoked row. Validation: Node `v24.19.0`; focused Vitest passed (2 files, 5 tests); TypeScript passed; changed-file ESLint passed; local browser read-back confirmed the disclosure and absence of the draft marker; production Next build passed (62 static pages); `git diff --check` passed. The focused Playwright check was attempted but credential-gated by a missing local Chromium binary. Deployment and final Google questionnaire submission remain pending.
