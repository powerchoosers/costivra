...
function databaseForDocument(document: { storage_path: string; status?: string; source_purged_at?: string | null } | null) {
...
describe("customer document download authorization", () => {
  ...
  it("returns gone without signing an original removed by retention", async () => {
    const database = databaseForDocument({
      storage_path: `${organizationId}/documents/invoice.pdf`,
      status: "ready",
