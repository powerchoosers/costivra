...
export async function ingestDocumentBuffer(input: {
  ...
  try {
    ...
    const { data: version, error: versionError } = await input.db.from("document_extraction_versions").insert({
      ...
      extractor_version: "costivra-intake-v1",
      provider: "openrouter",
      model_identifier: process.env.OPENROUTER_MODEL ?? "openai/gpt-4.1-mini",
      schema_version: "cost-document-v1",
    ...
    const finalStatus = intelligence.confidence < .75 ? "needs_review" : "ready";
    ...
    return { duplicate: false as const, documentId: document.id as string, extractionVersionId: version.id as string, status: finalStatus, sha256 };
  } catch (analysisError) {
    await input.db.from("document_extraction_versions").insert({ document_id: document.id, extractor_version: "costivra-intake-v1", provider: "openrouter", schema_version: "cost-document-v1", status: "failed", error_message: analysisError instanceof Error ? analysisError.message.slice(0, 1000) : "Extraction failed" });
