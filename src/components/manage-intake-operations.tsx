...
function DocumentRecoveryQueue({ documents }: { documents: ManageIntakeOperationsData["recoveryDocuments"] }) {
  ...
  return <section className="manage-panel intake-recovery-queue">
    ...
    <div className="invoice-review-table-wrap"><table className="invoice-review-table"><thead><tr><th>Client</th><th>Source file</th><th>Reading path</th><th>Reason</th><th>Received</th><th>Action</th></tr></thead><tbody>
      {documents.map((document) => <tr key={document.id}><td><strong>{document.organizationName}</strong></td><td><strong>{document.filename}</strong><small>{document.summary}</small></td><td>{document.inputMode === "pdf_ocr" ? "Image OCR" : "Embedded text"}</td><td>{document.recoveryState === "stalled" ? "Processing interrupted" : label(document.failureCode)}</td><td>{dateTime(document.createdAt)}</td><td><button className="manage-button" type="button" disabled={busyId !== null || !document.sourceAvailable} onClick={() => void retry(document.id)}><RefreshCw size={15} />{busyId === document.id ? "Recovering…" : document.sourceAvailable ? document.recoveryState === "stalled" ? "Recover job" : "Retry extraction" : "Source expired"}</button></td></tr>)}
    </tbody></table></div>
  </section>;
}
