...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const show = useCallback((input: ToastInput) => {
    ...
    setItems((current) => [...current.slice(-3), { id, title: input.title, message: input.message, tone, leaving: false }]);
  ...
  return <ToastContext.Provider value={value}>{children}<div className="toast-region" aria-live="polite" aria-label="Notifications">{items.map((item) => {
