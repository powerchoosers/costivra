...
const recordFieldGroups: Record<Kind, readonly RecordFieldGroupDefinition[]> = {
  ...
  invoice: [
    ...
    { id: "quality", label: "Matching & quality", description: "Deterministic reconciliation and identity checks that determine whether a review is needed.", keys: ["locationId", "reconciliationStatus", "vendorMatchStatus", "workspaceCustomerMatchStatus", "expenseAccountMatchStatus", "serviceLocationMatchStatus", "extractionConfidence"] },
  ],
  opportunity: [
...
function build(data: PortalData, kind: Kind, id: string) {
  ...
  if (kind === "invoice") {
    ...
    { key: "reviewNotes", label: "Reviewer notes", value: record.reviewNotes, editable, type: "textarea" }, { key: "locationId", label: "Service location", value: record.locationId ?? "", display: String(record.locationName ?? "All locations / not assigned"), editable, type: "select", options: [{ value: "", label: "All locations / not assigned" }, ...data.locations.map((location) => ({ value: location.id, label: `${location.name}${location.status === "inactive" ? " · archived" : ""}` }))] }, { key: "totalAmount", label: "Reconciled total", value: record.totalAmount, display: money(record.totalAmount as number | null, invoiceCurrency), note: "Protected extracted financial fact." },
