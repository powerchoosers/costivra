...
export function SequenceWorkspace({ data, query, mode = "sequences" }: Props) {
  ...
  const summary = useMemo(() => ({
    ...
    activeEnrollments: enrollments.filter((item) => ["pending", "active", "paused", "waiting_for_task"].includes(item.state)).length,
  ...
  async function sequenceAction(id: string, action: SequenceAction) {
    ...
    try {
      ...
      if (action === "clone" && typeof payload.id === "string") setSelectedId(payload.id);
      await load();
...
function SequenceEditor({ sequence, busy, activating, onActivate, onAdd, onSave, onReorder, onDelete, onDuplicate, onTestSend, onClone }: { sequence: Sequence; busy: boolean; activating: boolean; onActivate: () => void; onAdd: (type: SequenceStepType) => void; onSave: (patch: Record<string, unknown>) => void; onReorder: (stepIds: string[]) => void; onDelete: (stepId: string) => void; onDuplicate: (step: SequenceStep) => void; onTestSend: (stepId: string) => Promise<void>; onClone: () => void }) {
  ...
  const activation = sequenceActivationUiState(sequence.status, validation.valid, activating);
