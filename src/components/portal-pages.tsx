...
function BillingPanel() {
  ...
  const [status, setStatus] = useState<{ status?: string; providerConfigured?: boolean; billingMode?: "test" | "live" | "unknown"; billingEnabled?: boolean; setupReasons?: string[]; stripeAccount?: { reachable: boolean; chargesEnabled: boolean | null; payoutsEnabled: boolean | null; detailsSubmitted: boolean | null; currentlyDue: string[]; pastDue: string[]; disabledReason: string | null }; plans?: Array<{ key: string; name: string; description?: string; amountCents?: number | null; currency?: string; interval?: string; checkoutEnabled: boolean }>; subscriptions: Array<{ plan_key: string; status: string; billing_source?: "stripe" | "complimentary"; cancel_at_period_end: boolean; current_period_end: string | null }>; entitlements: Array<{ feature_key: string; enabled: boolean }> } | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
