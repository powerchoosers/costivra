...
function ScanPage() {
  return <PageFrame><div className="detail-layout" style={{ gridTemplateColumns: "1fr .85fr", alignItems: "start" }}><header className="content-hero"><h1>Start with three bills.</h1><p>Create a secure workspace, then upload your current bills or contracts. Costivra will store them privately, extract reviewable facts, and keep every finding tied to source evidence.</p><div style={{ marginTop: 44 }}><p><Check aria-hidden="true" size={17} style={{ color: "var(--mint-dark)", verticalAlign: "middle", marginRight: 8 }} />Private, organization-scoped storage</p><p><Check aria-hidden="true" size={17} style={{ color: "var(--mint-dark)", verticalAlign: "middle", marginRight: 8 }} />Duplicate detection and structured extraction</p><p><Check aria-hidden="true" size={17} style={{ color: "var(--mint-dark)", verticalAlign: "middle", marginRight: 8 }} />No vendor contact without approval</p></div></header><section className="panel" style={{ boxShadow: "var(--shadow)" }}><div className="panel-header"><h2>Free Cost Leak Scan</h2><span className="eyebrow">Secure account required</span></div><div className="panel-body" style={{ padding: 36 }}><ShieldCheck aria-hidden="true" size={34} style={{ color: "var(--blue)", marginBottom: 18 }}/><h2 style={{marginTop:0}}>Your source documents deserve a real security boundary.</h2><p className="muted" style={{lineHeight:1.65}}>Create your organization workspace first. After email confirmation, you can upload PDF, DOCX, or text records directly into private Supabase storage and see their extraction status.</p><Link className="button button-primary" href="/signup?next=/app/documents" style={{width:"100%",marginTop:18}}>Create a secure workspace <ArrowRight size={16}/></Link><Link className="button button-secondary" href="/login?next=/app/documents" style={{width:"100%",marginTop:10}}>I already have an account</Link></div></section></div></PageFrame>;
}
...
function AccountPage({ mode }: { mode: string }) {
  ...
  async function submitAccount(event: FormEvent<HTMLFormElement>) {
    ...
    const result = signup
      ? await client.auth.signUp({ email, password, options: { data: { full_name: String(form.get("fullName") ?? "").trim(), company_name: String(form.get("companyName") ?? "").trim() } } })
    ...
    router.replace(next?.startsWith("/app") ? next : "/app"); router.refresh();
  }
  return <PageFrame><div className="account-page"><aside className="account-intro"><div className="account-intro-mark"><span className="mini-mark">C</span><span>Costivra workspace</span></div><div><p className="eyebrow">A calmer way to control recurring cost</p><h1>{signup ? "Build your cost command center." : "Welcome back."}</h1><p>{signup ? "Start with a private workspace for bills, contracts, evidence, and decisions that need a clear owner." : "Review evidence, approve bounded actions, and keep the next decision in view."}</p></div><div className="account-trust"><span><LockKeyhole aria-hidden="true" size={16} /> Private by organization</span><span><BadgeCheck aria-hidden="true" size={16} /> Evidence stays attached</span></div></aside><form className="account-card" onSubmit={submitAccount}><div className="account-card-heading"><span className="account-card-kicker">{signup ? "Create workspace" : "Sign in"}</span><h2>{signup ? "Start with a secure account." : "Your workspace is ready."}</h2><p>{signup ? "Use your work email to create an organization-scoped workspace." : "Continue with the credentials associated with your Costivra workspace."}</p></div><div className="account-provider-grid" aria-label="Sign-in providers"><button className="account-provider" type="button" disabled aria-disabled="true"><span className="account-provider-icon">G</span><span>Continue with Google</span><small>Coming soon</small></button><button className="account-provider" type="button" disabled aria-disabled="true"><span className="account-provider-icon account-provider-icon--outlook">O</span><span>Continue with Outlook</span><small>Coming soon</small></button></div><div className="account-divider"><span>or use email</span></div>{signup && <div className="account-fields account-fields-double"><div className="field"><label htmlFor="full-name">Your name</label><input id="full-name" name="fullName" required autoComplete="name" /></div><div className="field"><label htmlFor="company-name">Company</label><input id="company-name" name="companyName" required autoComplete="organization" /></div></div>}<div className="account-fields"><div className="field"><label htmlFor="email">Work email</label><input id="email" name="email" required type="email" autoComplete="email" placeholder="you@company.com" /></div><div className="field"><div className="account-label-row"><label htmlFor="password">Password</label>{!signup && <Link href="/contact">Need help?</Link>}</div><input id="password" name="password" required type="password" minLength={10} autoComplete={signup ? "new-password" : "current-password"} /></div></div>{signup && <small className="account-hint">Use at least 10 characters.</small>}{message && <p role="alert" className="account-message">{message}</p>}<button className="button button-primary account-submit" type="submit" disabled={busy}>{busy ? "Working…" : signup ? "Create account" : "Sign in"}<ArrowRight aria-hidden="true" size={17} /></button><p className="account-switch">{signup ? <>Already have an account? <Link href="/login">Sign in</Link></> : <>New to Costivra? <Link href="/signup">Create an account</Link></>}</p></form></div></PageFrame>;
}
