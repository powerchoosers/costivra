...
function AccountPage({ mode }: { mode: string }) {
  ...
  return <PageFrame><form onSubmit={submitAccount} style={{ display: "grid", maxWidth: 480, margin: "30px auto", border: "1px solid var(--line)", background: "var(--white)", padding: 36, boxShadow: "var(--shadow)" }}><span className="mini-mark" style={{ marginBottom: 24 }}>C</span><h1 style={{ margin: 0, fontSize: "2.6rem", letterSpacing: "-.05em" }}>{signup ? "Create your Costivra account." : "Welcome back."}</h1><p className="muted" style={{ lineHeight: 1.6 }}>{signup ? "Start with a secure cost scan and build your recurring-expense command center." : "Sign in to review evidence, approve actions, and track verified value."}</p>{signup && <><div className="field" style={{ marginTop: 18 }}><label htmlFor="full-name">Your name</label><input id="full-name" name="fullName" required autoComplete="name" /></div><div className="field" style={{ marginTop: 16 }}><label htmlFor="company-name">Company</label><input id="company-name" name="companyName" required autoComplete="organization" /></div></>}<div className="field" style={{ marginTop: 16 }}><label htmlFor="email">Work email</label><input id="email" name="email" required type="email" autoComplete="email" placeholder="you@company.com" /></div><div className="field" style={{ marginTop: 16 }}><label htmlFor="password">Password</label><input id="password" name="password" required type="password" minLength={10} autoComplete={signup ? "new-password" : "current-password"} /></div>{signup&&<small className="muted" style={{marginTop:6}}>Use at least 10 characters.</small>}{message && <p role="alert" className="account-message">{message}</p>}<button className="button button-primary" type="submit" disabled={busy} style={{ marginTop: 20 }}>{busy ? "Working…" : signup ? "Create account" : "Sign in"}</button><p className="muted" style={{ textAlign: "center", fontSize: ".8rem", marginTop: 22 }}>{signup ? <>Already have an account? <Link href="/login" style={{ color: "var(--blue)" }}>Sign in</Link></> : <>New to Costivra? <Link href="/signup" style={{ color: "var(--blue)" }}>Create an account</Link></>}</p></form></PageFrame>;
}
