...
function ScanPage() {
  return <PageFrame><div className="detail-layout" style={{ gridTemplateColumns: "1fr .85fr", alignItems: "start" }}><header className="content-hero"><h1>Start with three bills.</h1><p>Create a secure workspace, then upload your current bills or contracts. Costivra will store them privately, extract reviewable facts, and keep every finding tied to source evidence.</p><div className="scan-assurances"><p className="scan-assurance"><Check aria-hidden="true" size={17} />Private, organization-scoped storage</p><p className="scan-assurance"><Check aria-hidden="true" size={17} />Duplicate detection and structured extraction</p><p className="scan-assurance"><Check aria-hidden="true" size={17} />No vendor contact without approval</p></div></header><section className="panel" style={{ boxShadow: "var(--shadow)" }}><div className="panel-header"><h2>Free Cost Leak Scan</h2><span className="eyebrow">Secure account required</span></div><div className="panel-body" style={{ padding: 36 }}><ShieldCheck aria-hidden="true" size={34} style={{ color: "var(--blue)", marginBottom: 18 }}/><h2 style={{marginTop:0}}>Your source documents deserve a real security boundary.</h2><p className="muted" style={{lineHeight:1.65}}>Create your organization workspace first. After email confirmation, you can upload PDF, DOCX, or text records directly into private Supabase storage and see their extraction status.</p><Link className="button button-primary" href="/signup?next=/app/documents" style={{width:"100%",marginTop:18}}>Create a secure workspace <ArrowRight size={16}/></Link><Link className="button button-secondary" href="/login?next=/app/documents" style={{width:"100%",marginTop:10}}>I already have an account</Link></div></section></div></PageFrame>;
}
