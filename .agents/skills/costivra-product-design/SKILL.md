...
- Author every new surface and state for both workspace themes from the start. Use the shared workspace tokens for the base contract, add an explicit `:root[data-workspace-theme="dark"]` treatment when a light default would leak into the dark shell, and verify nested additions such as inline confirmations, result rows, empty states, and skeletons in light and dark before calling the work complete. Never rely on an unexamined light-mode default inside a dark workspace.
