-- Packet 03: qualify table columns in privileged functions.
-- This keeps SECURITY DEFINER routines deterministic with an empty search_path
-- and prevents PL/pgSQL output/parameter names from becoming ambiguous.
...
create or replace function public.activate_crm_sequence(p_sequence_id uuid, p_actor_id uuid, p_now timestamptz default now())
returns table(sequence_id uuid, organization_id uuid, activated_at timestamptz, activated_enrollments integer)
language plpgsql set search_path = '' as $$
declare v_sequence public.crm_sequences%rowtype; v_activated_at timestamptz; v_activated_enrollments integer;
begin
  select s.* into v_sequence from public.crm_sequences as s where s.id = p_sequence_id and s.status in ('draft', 'paused') for update;
  if not found then return; end if;
  v_activated_at := coalesce(v_sequence.activated_at, p_now);
  update public.crm_sequences as s set status = 'active', execution_enabled = true, activated_at = v_activated_at, paused_at = null, updated_at = p_now where s.id = p_sequence_id;
  with activated as (
    update public.crm_sequence_enrollments as e set state = 'active', current_step_id = null, current_step_position = 0, next_action_at = p_now, started_at = p_now, paused_at = null, updated_at = p_now
    where e.sequence_id = p_sequence_id and e.state = 'pending' returning e.id, e.sequence_id
  ), inserted_events as (
    insert into public.crm_sequence_events(sequence_id, enrollment_id, event_type, safe_metadata, occurred_at)
    select a.sequence_id, a.id, 'step_scheduled', jsonb_build_object('next_action_at', p_now, 'activation', true), p_now from activated as a returning id
  ) select count(*) into v_activated_enrollments from inserted_events;
  insert into public.internal_audit_events(actor_id, organization_id, action, resource_type, resource_id, safe_metadata, created_at)
  values (p_actor_id, v_sequence.organization_id, 'crm.sequence_activated', 'crm_sequence', p_sequence_id, jsonb_build_object('execution_enabled', true, 'activated_enrollments', v_activated_enrollments), p_now);
  return query select p_sequence_id, v_sequence.organization_id, v_activated_at, v_activated_enrollments;
end;
$$;
