-- Preserve a reviewed lifecycle stage when archiving an account. Restoring an
-- account must not leave it visible while its lifecycle still says inactive.
...
begin
  ...
  if p_archived then
    ...
    if v_next_stage not in ('lead','onboarding','active','at_risk','inactive','closed') or v_next_stage = 'inactive' then v_next_stage := 'onboarding'; end if;
  end if;
  insert into public.crm_account_profiles (organization_id,lifecycle_stage,visible_in_crm,visibility_reason,updated_at)
  values (p_organization_id,v_next_stage,not p_archived,p_reason,now())
  on conflict (organization_id) do update set lifecycle_stage=excluded.lifecycle_stage,visible_in_crm=excluded.visible_in_crm,visibility_reason=excluded.visibility_reason,updated_at=now();
